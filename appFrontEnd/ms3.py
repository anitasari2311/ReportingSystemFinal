...
def runSchedule(kode_laporan):
    ...
    if jmlHead == '1':
        ...
        try:
            ...
            namaFileExcel =  kode_laporan+'_'+datetime.datetime.now().strftime('%Y%m%d')

            workbook = xlsxwriter.Workbook('/appReportingSystem/Schedule/%s.xls'% (namaFileExcel))
...
def testPreviewLaporan(kode_laporan):
    ...
    if jmlHead == '1':
        try:
            ...
            directory = 'C:/ExcelFile/'+tglSkrg

            if not os.path.exists(directory):
                os.makedirs(directory)

            workbook = xlsxwriter.Workbook(directory+'/%s.xls'% (kode_laporan))
            worksheet = workbook.add_worksheet()
